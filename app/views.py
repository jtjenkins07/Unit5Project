...
def home(request):
  ...
  for x in all_users:
    if x not in user_following_all:
      new_suggestions_list.append(x)
  current_user = User.objects.filter(username=request.user.username)

  final_suggestions_list = []
  for x in new_suggestions_list:
    if x not in current_user:
      final_suggestions_list.append(x)
...
def upload(request):
  if request.method == "POST":
    ...
    new_post = Post.objects.create(user=user, image=image, caption=caption, user_profile=user_profile)
    new_post.save()
...
def search(request):
  ...
  if request.method == "POST":
    username = request.POST['username']
    #Filters search by usernames containing search entry.
...
def admin_only(request):
  ...
  user_profile = Profile.objects.get(user=user_object)
  all_posts = Post.objects.all()
  messages.info(request, 'You are viewing an admin-only page.')
  return render(request, "admin.html", {"all_posts":all_posts, "user_profile":user_profile})